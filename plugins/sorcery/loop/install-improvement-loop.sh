...
echo
echo "Next:"
echo "  1. Commit pending scaffolding before running the loop. finish.sh runs"
echo "     'git add -A' at the end of each iteration, so anything untracked"
echo "     or modified when the loop starts gets swept into the first"
echo "     iteration's commit. Inspect git status, decide what's intentional,"
echo "     and commit it together — at minimum the loop infra, plus anything"
echo "     left over from earlier setup steps (e.g. .githooks/ from"
echo "     guarding-commits, claude.sh from launching-claude):"
echo "       git status"
