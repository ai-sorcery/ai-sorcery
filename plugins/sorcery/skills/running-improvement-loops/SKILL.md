...
- **Pins to `claude-opus-4-8[1m]`.** Edit `improvement/loop.sh` if a different model is wanted. The `[1m]` suffix enables the 1M-token context window — drop it for repos that don't need it.
